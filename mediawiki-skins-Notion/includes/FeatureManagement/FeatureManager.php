...
namespace MediaWiki\Skins\Notion\FeatureManagement;
 ...
 *
 * A "feature" is a friendly name for some component of the skin — however big or small — whose
 * availability depends on the state of the application: the reader's stored preferences, the
 * wiki's configuration, the title being rendered, or simply whether core has finished booting.
 * This class is the single place where those two vocabularies meet. Components ask for features by
 * name, the conditions behind them are registered separately as {@see Requirement}s, and neither
 * side has to change when the other does.
 *
 * Two responsibilities follow from that, and the second is the delicate one:
 *
 * - Evaluation. {@see self::isFeatureEnabled} answers whether *every* requirement of a feature is
 *   met. Requirements are evaluated in registration order and short-circuit on the first failure,
 ...
 * an evaluated result here would defeat that by design.
 *
 * The manager is request-scoped: `includes/ServiceWiring.php` builds one per request through the
 * feature-manager factory, which registers requirements first and the features that reference them
 * second. That order is enforced rather than assumed — see {@see self::registerFeature}.
 *
 * The class is deliberately not declared `final` even though it is documented as such, so that
 * PHPUnit can create mocks of it for the component unit tests.
 *
 ...
 * @unstable
 *
 * @package MediaWiki\Skins\Notion\FeatureManagement
 * @internal
 * @final
 */
class FeatureManager {
	...
	/**
	 * @param ConfigHelper $configHelper Answers whether a page-sensitive feature is excluded on the
	 *   page being rendered. Only the configurable font size consults it.
	 * @param UserOptionsLookup $userOptionsLookup Reads the reader's stored preference values.
	 * @param IContextSource $context The request context the feature state is evaluated against.
	 */
	 ...
	 *
	 * Every referenced requirement must already be registered. That rule is load-bearing: it is
	 * what obliges the feature-manager factory to register requirements before the features built
	 * on top of them, turning a silently mis-ordered boot into an immediate exception.
	 *
	...
	public function getUserPreferenceValue( $preferenceKey ) {
		return $this->userOptionsLookup->getOption(
			...
			// in 'resources/skins.notion.js/clientPreferences.json'
		);
	}
	 ...
	 * hyphenated lower case, and a suffix describing the current state. Night mode is the single
	 * exception and drops the skin-specific namespace entirely — see the comment on its case below.
	 *
	 * The suffix vocabulary is not decorative either. A `clientpref-` suffix marks a class the
	 * client-preference scripts are allowed to rewrite in the browser, which is why the flags that
	 * can only be changed on the server use `enabled`/`disabled` instead: it keeps `features.js`
	 * from being pointed at a class no amount of clicking can actually change.
	 *
	 * @return array
	 * @throws RuntimeException If a registered feature has no case below, i.e. the feature and this
	 *  method have drifted apart
	 */
	public function getFeatureBodyClass() {
		return array_map( function ( $featureName ) {
			...
			$prefix = 'notion-feature-' . $featureClass . '-';
			...
			switch ( $featureName ) {
				...
				case Constants::FEATURE_FONT_SIZE:
					if ( $this->configHelper->shouldDisable(
						$config->get( Constants::CONFIG_KEY_FONT_SIZE_CONFIGURABLE_OPTIONS ), $request, $title
					) ) {
	 ...
	 *
	 * Requirements are evaluated in the order they were registered against the feature and the
	 * first unmet one wins, so no later requirement is consulted once the answer is known.
	 *
